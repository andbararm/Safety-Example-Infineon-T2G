...
static const ARM_MPU_Region_t mpu_table[ZONES_NUM][MPU_REGIONS] = {
  ...
  /* Zone 'Zone_Com' */ 
  {
    ...
    // RAM for interrupt handler addresses (0x280049C0..0x280049FF) - required by Cy_SysInt_Init function used in Ethernet driver PowerControl function
    // It would be better if interrupt vectors would be in flash thus would not be prone to tampering.
    // Limitation of MPU that minimum area size is 32 bytes and has to be properly aligned is not suitable for protecting vectors in RAM.
    { .RBAR = ARM_MPU_RBAR(3U, 0x280049C0), .RASR = ARM_MPU_RASR_EX(1U, ARM_MPU_AP_FULL, ARM_MPU_ACCESS_NORMAL(ARM_MPU_CACHEP_NOCACHE, ARM_MPU_CACHEP_NOCACHE, 0U), 0x00U, ARM_MPU_REGION_SIZE_64B)  },
    // CPUSS (0x40200000..0x4020FFFF) - required by Cy_SysInt_Init function used in Ethernet driver PowerControl function
    // It would be better if system (interrupts) was initialized before RTOS starts because at that point there are no restrictions.
    { .RBAR = ARM_MPU_RBAR(4U, 0x40200000), .RASR = ARM_MPU_RASR_EX(1U, ARM_MPU_AP_FULL, ARM_MPU_ACCESS_DEVICE(1U),                                                 0x00U, ARM_MPU_REGION_SIZE_64KB) },
    // ETH0 (0x40480000..0x4048FFFF)
    { .RBAR = ARM_MPU_RBAR(5U, 0x40480000), .RASR = ARM_MPU_RASR_EX(1U, ARM_MPU_AP_FULL, ARM_MPU_ACCESS_DEVICE(1U),                                                 0x00U, ARM_MPU_REGION_SIZE_64KB) },
    { .RBAR = ARM_MPU_RBAR(6U, 0U), .RASR = 0U },
    { .RBAR = ARM_MPU_RBAR(7U, 0U), .RASR = 0U }
  },
  /* Zone 'Zone_Safe_OP' */ 
