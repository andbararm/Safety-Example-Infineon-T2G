...
struct RxFrameBuffer {
  ...
  /* It is safe to place the frame length in the last 4 bytes of the buffer.     */
...
static void eth0_rx_frame_cb (ETH_Type *base, uint8_t *rx_buffer, uint32_t length) {
  ...
  Rx_Buf[emac.rx_head].Length = length;
...
static int32_t PowerControl (ARM_POWER_STATE state) {
  ...
  switch (state) {
    case ARM_POWER_OFF:
      ...
      ETH_CTL(emac.base) &= ~ETH_CTL_ENABLED_Msk;
    ...
    case ARM_POWER_FULL:
      ...
      /* Remove FCS from received frames */
      emac.drv->setFcsRemove (emac.pd, 1U);

      /* Drop CRC error frames */
      emac.drv->setIgnoreFcsRx (emac.pd, 0U);
...
/**
  \fn          uint32_t hash_get_index (uint8_t *addr)
  \brief       Calculate hash index value for the specified address.
  \param[in]   mac_addr  pointer to address
  \return      Calculated hash index
*/
static uint32_t hash_get_index (const uint8_t *addr) {
  uint32_t i, j, bit_val;
  uint32_t hash_index = 0;

  for (j = 0U; j < 6U; j++) {
    for (i = 0U, bit_val = 0U; i < 8U; i++) {
      uint32_t bit = i * 6U + j;
      if (addr[bit >> 3U] & (1U << (bit & 0x7))) {
        bit_val ^= 0x00000001;
      }
    }
    hash_index |= (bit_val << j);
  }
  return (hash_index);
}
...
static int32_t SetAddressFilter (const ARM_ETH_MAC_ADDR *ptr_addr, uint32_t num_addr) {
  ...
  if (num_addr == 2U) {
    /* Use specific address filters 2, 3 */
    emac.drv->setSpecificAddr (emac.pd, CY_ETHIF_FILTER_NUM_4, &mac_null, 0U, 0U);
    return ARM_DRIVER_OK;
  }
  ptr_addr++;
  ...
  if (num_addr == 3U) {
    /* Use specific address filters 2, 3, 4 */
    return ARM_DRIVER_OK;
  }
  ptr_addr++;

  /* Calculate 64-bit Hash table for the remaining MAC addresses */
  hash_table[0] = hash_table[1] = 0U;
  for ( ; num_addr > 3U; ptr_addr++, num_addr--) {
    uint32_t bit = hash_get_index (ptr_addr->b);
    hash_table[bit >> 5] |= 1 << (bit & 0x1F);
  }

  /* Setup multicast hash filtering */
  emac.drv->setHashAddr (emac.pd, hash_table[1], hash_table[0]);
  ...
  switch (control) {
    case ARM_ETH_MAC_CONFIGURE:
      ...
      if (arg & ARM_ETH_MAC_LOOPBACK) {
        /* The Cadence driver accesses the PCS register that is removed, */
        /* so the 'setLoopback()' function generates a Fault exception.  */
//      emac.drv->setLoopback (emac.pd, CEDI_LOCAL_LOOPBACK);
        emac.base->NETWORK_CONTROL |= 0x00000002;
      }
      else {
//      emac.drv->setLoopback (emac.pd, CEDI_NO_LOOPBACK);
    ...
    case ARM_ETH_MAC_VLAN_FILTER:
      ...
      if (arg != 0U) {
        ...
        /* Not supported by EMAC, dump only non-VLAN tagged frames */
